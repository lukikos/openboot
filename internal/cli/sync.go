...
var syncCmd = &cobra.Command{
	Use:   "sync",
	Short: "Sync your system with a remote config",
...
interactively select which changes to apply.`,
  ...
  # Only install missing packages, skip removal prompts
  openboot sync --install-only`,
	SilenceUsage: true,
...
func runSync(cmd *cobra.Command) error {
	...
	// 5. Check if in sync
	if !diff.HasChanges() {
		ui.Success("Your system is in sync!")
